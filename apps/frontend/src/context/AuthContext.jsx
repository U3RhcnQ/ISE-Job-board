...
export const AuthProvider = ({ children }) => {
    ...
    const fetchUserProfile = useCallback(async (currentToken) => {
        ...
        try {
            ...
            const accessResponse = await fetch(`${API_BASE_URL}/access`, { // Replace with your actual access endpoint
                headers: { 'Authorization': `Bearer ${currentToken}` },
            });

            if (!accessResponse.ok) {
                const errorData = await accessResponse.json().catch(() => ({}));
                throw new Error(errorData.message || `Failed to fetch access level (status: ${accessResponse.status})`);
            }
            ...
            if (current_access_level === 'rep') {
                ...
                const companyInfoResponse = await fetch(`${API_BASE_URL}/rep-info?user_id=${rawUserData.internal_user_id}`, { // Example endpoint
                    headers: { 'Authorization': `Bearer ${currentToken}` },
                });
                if (!companyInfoResponse.ok) {
                    const errorData = await companyInfoResponse.json().catch(() => ({}));
                    throw new Error(errorData.message || `Failed to fetch company info for rep (status: ${companyInfoResponse.status})`);
                }
                ...
                if (!rawUserData.internal_user_id) {
                    console.warn("Student ID not found in profile data to fetch student-specific info.");
                } else {
                    console.log(`Fetching /student-info for student ID: ${rawUserData.internal_user_id}`);
                    const studentInfoResponse = await fetch(`${API_BASE_URL}/student-info?user_id=${rawUserData.internal_user_id}`, { // Example
                        headers: { 'Authorization': `Bearer ${currentToken}` },
                    });
                    if (!studentInfoResponse.ok) {
                        const errorData = await studentInfoResponse.json().catch(() => ({}));
                        throw new Error(errorData.message || `Failed to fetch student info (status: ${studentInfoResponse.status})`);
                    }
                ...
                console.log("Admin user. Additional data fetching can be added here if needed.");
                // Example:
            ...
            if (!processedUser.access_level) {
                console.error("Critical error: Processed user is missing access_level.", processedUser, rawUserData);
    ...
    useEffect(() => {
        ...
        if (token && !user) { // Only fetch if token exists and user isn't already loaded
            console.log("AuthProvider Token/User Effect: Token present, user not loaded. Fetching profile.");
            fetchUserProfile(token).then(fetchedUser => {

                if (fetchedUser && fetchedUser.access_level) { // Check for fetchedUser and access_level
                    ...
                    if (pathsToRedirectFrom.includes(currentPath)) {
                        switch (fetchedUser.access_level) {
                            ...
                            case 'student':
                                ...
                                break;
                            default:
                                console.warn(`Post-fetch: Unknown access level: ${fetchedUser.access_level}. Logging out.`);
                    ...
                    clearAuthData("User profile fetched but critical access level information is missing.");
                    navigate('/login');
                }
    ...
    }, [token, user, fetchUserProfile, navigate, clearAuthData]);
