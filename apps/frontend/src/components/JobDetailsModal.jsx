...
const JobDetailsModal = ({ jobId, onOperationSuccess }) => {
    ...
    const canEditPermissions = user && (user.access_level === 'admin' || user.access_level === 'rep');
    ...
    useEffect(() => {
        ...
        fetchJobDetails();
    }, [jobId, token, isCreateMode]); // Rerun if these change
    ...
    const handleToggleEditState = () => {
        ...
        if (!isInEditState) { // Entering edit form
    ...
    const handleCancelOperation = () => {
        ...
        } else { // Edit mode: cancel the edit, revert to view state
    ...
    const handleSaveChanges = async () => {
        ...
        if (isCreateMode && (formData.company_id === null || formData.company_id === undefined || String(formData.company_id).trim() === "")) {
            console.warn("DEBUG: Company ID validation FAILED for create mode. company_id:", formData.company_id); // LOG C
        ...
        const mapApprovalToApiInt = (approvalString) => {
            switch (approvalString?.toLowerCase()) {
                ...
                case 'pending':
                default: return null; // API: blank/NULL - pending
            }
        };

        // Prepare salary as integer or null
        const salaryInt = formData.salary !== null && formData.salary !== undefined && !isNaN(parseFloat(String(formData.salary)))
        ...
        const positionCountInt = formData.position_count !== null && formData.position_count !== undefined && !isNaN(parseInt(String(formData.position_count),10))
        ...
        if (isCreateMode) {
            ...
            payload = {
                ...
                residency: formData.residency === '__NONE__' ? null : formData.residency,
                residency_title: formData.residency_title,
                approval: mapApprovalToApiInt(formData.approval),
            };
    ...
    if (isLoading) { // Primarily for fetching in edit mode
        return (
            <DialogContent><DialogHeader><DialogTitle>Loading...</DialogTitle></DialogHeader>
    ...
    if (error && !isCreateMode && !viewData && !isLoading) {
        return (
            <DialogContent><DialogHeader><DialogTitle className="text-red-600">Error</DialogTitle></DialogHeader>
    ...
    if (!isCreateMode && !viewData && !isLoading) {
        return (
            <DialogContent><DialogHeader><DialogTitle>Not Found</DialogTitle></DialogHeader>
        ...
        <DialogContent className="w-[95vw] max-w-[95vw] md:w-[80vw] md:max-w-[80vw] lg:w-[70vw] lg:max-w-[70vw] h-[90vh] flex flex-col">
            <DialogHeader className="m-6 mb-2">
                <DialogTitle className="text-2xl flex items-center justify-between">
                    <span>
                        {isCreateMode ? "Create New Job Listing" :
                            (isInEditState && formData) ? `${formData.company_name || "Edit Job"} - ${formData.job_title || ""}` :
                    ...
                    {!isCreateMode && displayDataForViewMode.approval && user?.access_level !== "student" && (
                        <div className="flex items-center space-x-1.5 ml-3">
                            ...
                            <Badge
                                variant={getApprovalBadgeVariant(displayDataForViewMode.approval)}
                                className={`ml-2 text-xs font-medium rounded-full px-4 py-2 ${
                                    displayDataForViewMode.approval?.toLowerCase() === 'approved' ? 'bg-green-100 text-green-700' :
                                        displayDataForViewMode.approval?.toLowerCase() === 'pending' ? 'bg-yellow-100 text-yellow-700' :
                                            displayDataForViewMode.approval?.toLowerCase() === 'rejected' ? 'bg-red-100 text-red-700' :
                                                'bg-gray-100 text-gray-700'
                                }`}
                            >
                                {displayDataForViewMode.approval?.charAt(0).toUpperCase() + displayDataForViewMode.approval?.slice(1) || 'Unknown'}
                            </Badge>
                        </div>
                    )}
                </DialogTitle>
            ...
            <div className="flex-grow overflow-y-auto p-6 pt-0 space-y-6">
                ...
                {error && isSubmitting && <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4" role="alert">{error}</div>}
                ...
                <section>
                    <h3 className="text-xl font-semibold text-gray-800 mb-3 border-b pb-2">Job Overview</h3>
                    <div className="space-y-1">
                        {(isInEditState || isCreateMode) && canEditPermissions ? (
                            <> {/* EDIT/CREATE FORM FIELDS */}

                                {isCreateMode && user?.access_level === 'rep' && formData?.company_name && (
                                ...
                                <FormField label="Company ID" htmlFor="company_id">
                                    <Input
                                        ...
                                        name="company_id"
                                        id="company_id"
                                        value={ user?.access_level === 'admin' ? formData?.company_id : user.company_id}
                                        onChange={(e) => handleIntegerInputChange("company_id", e.target.value)}
                                        disabled={
                                        ...
                                        placeholder={user?.access_level === 'admin' && isCreateMode ? "Enter Company ID" : user.company_id || ""}
                                    />
                                </FormField>

                                <FormField label="Website" htmlFor="website"><Input name="website" id="website" value={formData?.website || ''} onChange={handleInputChange} placeholder="https://example.com" /></FormField>
                                <FormField label="Job Title" htmlFor="job_title"><Input name="job_title" id="job_title" value={formData?.job_title || ''} onChange={handleInputChange} /></FormField>
                                <FormField label="Salary (€ per/month)" htmlFor="salary">
                                    <NumericFormat name="salary" id="salary" value={formData?.salary || ''} customInput={Input} thousandSeparator="," decimalSeparator="." allowNegative={false} decimalScale={2} fixedDecimalScale onValueChange={(values) => handleNumericInputChange("salary", values.floatValue)} />
                                </FormField>
                                <FormField label="Residency / Type" htmlFor="residency_title"><Input name="residency_title" id="residency_title" value={formData?.residency_title || ''} onChange={handleInputChange} /></FormField>
                                <FormField label="Residency Tag" htmlFor="residency">
                                    <Select name="residency" value={formData?.residency || ''} onValueChange={(value) => handleSelectChange("residency", value === '__NONE__' ? null : value)}>
                                        <SelectTrigger id="residency"><SelectValue placeholder="Select residency tag" /></SelectTrigger>
                                        <SelectContent>{residencySelectOptions.map(opt => <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>)}</SelectContent>
                                    </Select>
                                </FormField>
                                <FormField label="Positions Available" htmlFor="position_count"><Input type="number" name="position_count" id="position_count" value={formData?.position_count || ''} onChange={(e) => handleIntegerInputChange("position_count", e.target.value)} min="0" /></FormField>
                            </>
                        ) : ( // VIEW MODE (for existing job)
                            <>
                                <InfoItem label="Company">{displayDataForViewMode.company_name}</InfoItem>
                                {displayDataForViewMode.website && <InfoItem label="Website"><a href={displayDataForViewMode.website.startsWith('http') ? displayDataForViewMode.website : `//${displayDataForViewMode.website}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">{displayDataForViewMode.website}</a></InfoItem>}
                                <InfoItem label="Job Title">{displayDataForViewMode.job_title}</InfoItem>
                                <InfoItem label="Salary"><NumericFormat value={displayDataForViewMode.salary} displayType="text" thousandSeparator="," decimalSeparator="." prefix="€ " decimalScale={2} fixedDecimalScale /> per/month</InfoItem>
                                <InfoItem label="Residency / Type">{displayDataForViewMode.residency_title || 'Not specified'}</InfoItem>
                                <InfoItem label="Residency Tag">{displayDataForViewMode.residency ? displayDataForViewMode.residency.toUpperCase() : 'N/A'}</InfoItem>
                                <InfoItem label="Positions Available">{displayDataForViewMode.position_count}</InfoItem>
                            </>
                        )}
                    </div>
                </section>
                ...
                <section>
                    <h3 className="text-xl font-semibold text-gray-800 mb-3 border-b pb-2">Job Description</h3>
                    {(isInEditState || isCreateMode) && canEditPermissions ? (
                        <>
                            <FormField label="Full Description" htmlFor="description">
                                <Textarea name="description" id="description" value={formData?.description || ''} onChange={handleInputChange} rows={5} />
                            </FormField>
                            <FormField label="Short Description (for card)" htmlFor="small_description">
                                <Textarea name="small_description" id="small_description" value={formData?.small_description || ''} onChange={handleInputChange} rows={3} />
                            </FormField>
                        </>
                    ) : (
                        <p className="text-sm text-gray-700 whitespace-pre-wrap bg-gray-50 p-4 rounded-md shadow-sm leading-relaxed">{displayDataForViewMode.description}</p>
                    )}
                </section>
                ...
                {user?.access_level === "admin" && ( // Only admins can see/edit approval status section
                    <section>
                        <h3 className="text-xl font-semibold text-gray-800 mb-3 border-b pb-2">Admin Status</h3>
                        <div className="space-y-1">
                            {(isInEditState || isCreateMode) ? ( // In create mode, admin can set initial approval. In edit, admin can change.
                                <>
                                    <FormField label="Approval Status" htmlFor="approval">
                                        <Select name="approval" value={formData?.approval || 'pending'} onValueChange={(value) => handleSelectChange("approval", value)}>
                                            <SelectTrigger id="approval"><SelectValue placeholder="Select status" /></SelectTrigger>
                                            <SelectContent>{approvalOptions.map(opt => <SelectItem key={opt} value={opt}>{opt.charAt(0).toUpperCase() + opt.slice(1)}</SelectItem>)}</SelectContent>
                                        </Select>
                                    </FormField>
                                </>
                            ) : ( // View mode for approval status
                                <InfoItem label="Approval Status">
                                    <Badge variant={getApprovalBadgeVariant(displayDataForViewMode.approval)} className={`px-2.5 py-0.5 text-xs font-medium rounded-full ${
            ...
            <DialogFooter className="mt-auto pt-4 pb-4 pr-6 border-t flex justify-end space-x-2">
                {isCreateMode && canEditPermissions && ( // Buttons for CREATE mode
                    <>
                        <DialogClose asChild><Button type="button" variant="outline" onClick={handleCancelOperation} disabled={isSubmitting}>Cancel</Button></DialogClose>
                        <Button type="button" onClick={handleSaveChanges} className={"bg-green-600 hover:bg-green-700"} disabled={isSubmitting}>
                            {isSubmitting ? 'Creating...' : 'Create Job'}
                        </Button>
                    </>
                )}
                {!isCreateMode && canEditPermissions && ( // Buttons for EDIT mode
                    isInEditState ? ( // Form is active for editing
                        <>
                            <Button type="button" variant="outline" onClick={handleCancelOperation} disabled={isSubmitting}>Cancel Edit</Button>
                            <Button type="button" onClick={handleSaveChanges} className={"bg-green-600 hover:bg-green-700"} disabled={isSubmitting}>
                                {isSubmitting ? 'Saving...' : 'Save Changes'}
                            </Button>
                        </>
                    ) : ( // Viewing existing job details
                        <Button type="button" variant="default" className={"bg-green-600 hover:bg-green-700"} onClick={handleToggleEditState}>Edit</Button>
                    )
                )}
